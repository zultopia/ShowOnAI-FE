...
const typography = {
  ...
  h2: "font-manrope font-bold text-2xl md:text-3xl lg:text-4xl xl:text-5xl leading-tight",
  h3: "font-manrope font-semibold text-xl md:text-2xl leading-relaxed",
  h4: "font-manrope font-semibold text-lg md:text-xl leading-relaxed",
  title: "font-manrope font-semibold text-2xl md:text-3xl leading-relaxed",
  title2: "font-manrope font-semibold text-xl md:text-2xl leading-relaxed",
  subtitle: "font-manrope font-normal text-lg md:text-xl lg:text-2xl leading-relaxed",
...
const HeroSection = ({ email, setEmail, onEmailSubmit }: { email: string, setEmail: (email: string) => void, onEmailSubmit: () => void }) => (
  <section className="relative pt-8 md:pt-16 pb-12 md:pb-24 px-4 sm:px-6 lg:px-8">
    <div className="relative z-10 max-w-5/6 mx-auto text-center">
      ...
      <h1 className={`${typography.h1} text-white mb-6 max-w-5xl mx-auto`}>
        국내 시장의 AI 검색 최적화를 위한<br />
        단 하나의 솔루션, ShowOnAI
      </h1>

      <p className={`${typography.subtitle} text-gray-200 mb-12 max-w-5xl mx-auto`}>
        국내 사용자 데이터와 검색 환경을 기반으로, 브랜드에 최적화된 SEO 전략을 수립합니다.<br className="hidden md:block" />
        자체 R&D를 통해 축적한 SEO 기술과 실행 중심 컨설팅으로 결과를 만듭니다.
      </p>
      
      <div className="flex justify-center">
        <div className="flex items-center w-full max-w-lg md:max-w-2xl h-12 md:h-14 px-1 md:px-2 rounded-full border border-gray-200 bg-white shadow-2xl">
          <input
            type="email"
            placeholder="회사 이메일을 입력하세요"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            className="flex-1 ml-2 md:ml-4 bg-transparent border-none outline-none text-gray-600 placeholder-gray-400 text-sm md:text-lg"
          />
          <button
            onClick={onEmailSubmit}
            className="ml-2 px-4 md:px-6 py-1 md:py-2 rounded-full bg-black text-white font-semibold text-sm md:text-lg hover:bg-gray-800 transition-colors"
          >
            무료 진단 받기
          </button>
        </div>
...
const CompanyLogos = () => {
  ...
  return (
    <section className="relative py-12 md:py-20 overflow-hidden">
      <div className="flex items-center space-x-12 md:space-x-24 animate-scroll-rtl">
        {[...logos, ...logos].map((logo, index) => (
          <img 
            key={index}
            src={logo.src} 
            alt={logo.alt} 
            className="h-4 md:h-10 w-auto opacity-70 hover:opacity-100 transition-opacity" 
          />
        ))}
      </div>
      <style jsx>{`
...
const FAQSection = ({ expandedFaq, setExpandedFaq }: { expandedFaq: number[], setExpandedFaq: (expandedFaq: number[]) => void }) => {
  ...
  return (
    <section className="relative py-12 md:py-24 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5/6 mx-auto">
        <div className="text-center mb-12 md:mb-16">
          <p className={`${typography.caption} text-blue-600 mb-4`}>FREQUENTLY ASKED QUESTIONS</p>
          <h2 className={`${typography.h2} text-gray-900 mb-4`}>자주 묻는 질문</h2>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          {faqs.map((faq, index) => (
            <div key={index} className={index < faqs.length - 1 ? 'border-b border-gray-200' : ''}>
              ...
              >
                <h3 className="text-neutral-950 font-semibold text-xl md:text-2xl pr-4">{faq.question}</h3>
                <span className="text-neutral-700 text-xl md:text-2xl font-light flex-shrink-0 bg-primary-blue-50 rounded-md px-2">
                  {expandedFaq.includes(index) ? '−' : '+'}
                </span>
              </button>
              {expandedFaq.includes(index) && faq.answer && (
                <div className="px-6 pb-6 border-t border-gray-100">
                  <div className="text-gray-600 leading-relaxed pt-4 whitespace-pre-line">
                    {faq.answer}
                  </div>
                </div>
              )}
